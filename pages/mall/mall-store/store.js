...
Page({
  ...
  toCateStore(e){
    ...
    wx.navigateTo({
      url: '/pages/mall/mall-store/store-cate/cate?cateId='+cateId,
  ...
  //2/banner/addsList
  getStoreBanner(){
    let url1 = app.globalData.baseUrl + 'apiUser/banner/storeList'
    util.getRequestList(url1, false, this.storeBanner1Res)
    let url2 = app.globalData.baseUrl + 'apiUser/banner/addsList'
    util.getRequestList(url2, false, this.storeBanner2Res)
  },
  storeBanner1Res(res,type){
    if(res.data.code==='200'){
      let list=res.data.content
      if(list.length){
        this.setData({banner1List:list})
      }
    }
  },
  storeBanner2Res(res,type){
    if(res.data.code==='200'){
      let list=res.data.content
      if(list.length){
        this.setData({banner2List:list})
      }
    }
  },
  //获取商城分类列表
