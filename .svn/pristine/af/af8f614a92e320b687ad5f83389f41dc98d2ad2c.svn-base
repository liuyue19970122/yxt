...
Page({
  ...
  formatDateDay(n,time){
    let date = new Date()
    let ms=date.getTime()
    let addMs=ms+n*24*60*60*1000
    let curDate=util.formatDate(addMs)
    let deliverTime = curDate +' '+ time
  ...
  //是否自提
  switchChange(e){
    this.setData({isBySelf:e.detail})
  },
  //bindSelTime
  bindSelTime(e){
    let deliverTime = ''
  ...
  //提交订单///order/inst/add
  //addressId,deliverTime,isBySelf
  ...
  onSubmitOrder(){
    ...
    let isBySelf=this.data.isBySelf?'1':'0'
  ...
  onLoad: function(options) {
    ...
    let curDate = util.formatDate(new Date())
    let deliverTime = curDate + ' ' + chooseTimeArr[0].time
    //deliverTimeStr
    let deliverTimeStr = dayList[0].name + chooseTimeArr[0].time
    this.setData({ deliverTimeStr, deliverTime})
    let pageInType=options.pageInType
    if(pageInType==='buyAgain'){
      let infoStr=options.infoStr
      let infoStr1=infoStr.replace(/\*/g,'?')
      let infoStr2=infoStr1.replace(/\#/g,'=')
      let infoStr3=infoStr2.replace(/\$/g,'&')
      let info=JSON.parse(infoStr3)
      let totalMoney = options.totalMoney
      let orm = util.accMul(totalMoney,100)
      let orderMoney=Number(orm)
      let favMoney = 0
      let cpl=info.goodsList
      let orgId=info.orgId
      for (let i = 0; i < cpl.length; i++) {
        cpl[i].cusBuyCount = cpl[i].buyCount
        cpl[i].cusSalePrice =  cpl[i].orderMoney
        cpl[i].orgId=orgId
      }
      let npcl = []
      let obj = {
