  ...
  */
  ...
  goToSubmit(){
    ...
    if(sl.length){
      ...
      sl.forEach(item=>{
        data.goodsIds.push(item.keyId)
      })
      this.importFoods(data)
    }else{
      wx.showToast({
        title: '请选择货品',
        icon:'none'
  ...
  //导入商品请求/store/default/initByGoods
  //goodsIds 模版Id数组
  importFoods(data){
    let url = app.globalData.baseUrl + 'apiMall/store/default/initByGoods'
    util.postRequestList(url,data, false, this.importFoodsRes)
  },
  importFoodsRes(res,type){
  ...
  //获取系统默认货品///store/default/list
  getFoodList(data,type) {
    wx.showLoading({
      title: '获取中...',
    })
    let url = app.globalData.baseUrl + 'apiMall/store/default/list?t=' + times
