// pages/mall/handle-order/supplier-add/add.js
...
Page({
  ...
  //添加供货商/stock/order/newProducer
  //name,linkName,mobile,address
  addSupplierRequest(data){
    let url = app.globalData.baseUrl +'apiStock/stock/order/newProducer'
    util.getRequestListData(url,data,false,this.addSupplierRes)
  },
  addSupplierRes(res,type){
    if(res.data.code==='200'){
      ...
      supplierList.push(resObj)
      let orderInfo = prevPage.data.orderInfo
      orderInfo.producerId = resObj.keyId
      prevPage.setData({
        orderInfo,
        supplierList,
        supplierIndex:0,
        selSupName: resObj.name
      })
      wx.navigateBack({
        delta:1
      })
